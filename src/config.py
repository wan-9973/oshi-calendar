...
# --- 関連度の広がりすぎ対策（個人が推し本人の新刊・新譜・発売予定を見つけやすくする） ---
# 「幅が広すぎる関連商品」（中古・レンタル落ち・楽譜/スコア・カラオケ等の周辺商品）は、
# 推し本人の公式な供給とは性質が異なるため、既定で表示から除外する。
# タイトル/著者アーティスト欄にこれらの語を含む商品が対象。誤除去時は
# 環境変数 OSHI_FILTER_NOISE=0 で一括無効化できる（スイッチ）。
NG_NOISE_WORDS = [
    # 中古・レンタル・まとめ売り（非公式の再販）
    "中古", "レンタル落ち", "レンタルアップ", "まとめ売り", "まとめ買い",
    # 楽譜・スコア類（本人名義でない周辺商品）
    "楽譜", "ピアノピース", "ピアノ・ピース", "ピアノソロ", "バンドスコア",
    "ギターピース", "スコアブック", "コーラスピース", "メロディ譜",
    # カラオケ・ものまね等
    "カラオケ", "ものまね",
]
FILTER_NOISE = os.environ.get("OSHI_FILTER_NOISE", "1") != "0"
# 楽天APIのNGKeywordに渡す除外語（キーワード検索系のichiba/books_totalで送信）。
# API段階で明確なノイズ（中古・レンタル・楽譜・カラオケ）を減らし、
# コード側フィルタ(NG_NOISE_WORDS)と二重に守る。
NG_SEARCH_KEYWORDS = os.environ.get("OSHI_NG_SEARCH_KEYWORDS", "中古 レンタル 楽譜 カラオケ")
# 説明文（itemCaption）だけに名前が出る商品は関連が薄く「幅が広すぎる」ため、
# 信頼フィールド検索（author/artistName）以外では既定で非表示にする。
SHOW_CAPTION_ONLY = os.environ.get("OSHI_SHOW_CAPTION_ONLY", "0") != "0"
